...
* **Recipe and kitchen-fit assistance:** Users can provide the ingredients and cooking equipment they have in chat. Before recommending a recipe, the model can call a server-owned `checkKitchen` tool that deterministically compares recipe needs against what the user has said they own. Missing gear returns a workaround or an alternate-recipe prompt rather than a dead-end refusal. The tool does not assume a universal starter kitchen. A dedicated local equipment profile UI is still out of this time box.
